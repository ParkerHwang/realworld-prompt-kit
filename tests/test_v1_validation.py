...
from tools.v1_validation import (
    DEFAULT_V1_MANIFEST,
    ROOT,
    _quality_lint,
...
class V1ValidationTests(unittest.TestCase):
    def test_catalog_has_authorized_vocabularies(self) -> None:
        manifest = load_json(DEFAULT_V1_MANIFEST)
    ...
    def test_v1_repository_is_valid(self) -> None:
        report = validate_v1_repository()
    ...
    def test_similarity_lint_rejects_wrapper_identity(self) -> None:
        manifest = load_json(DEFAULT_V1_MANIFEST)
        scenario_path = sorted((ROOT / "data/v1.0/scenarios").rglob("*.json"))[0]
    ...
    def test_v1_schema_is_valid_json(self) -> None:
        with (ROOT / "schemas/scenario.v1.schema.json").open(encoding="utf-8") as handle:
            schema = json.load(handle)
        self.assertEqual(schema["$schema"], "https://json-schema.org/draft/2020-12/schema")
        self.assertEqual(schema["properties"]["schema"]["const"], "realworld-prompt-kit.scenario/1.0.0")
    ...
    def test_profile_realism_rejects_non_terse_prompt(self) -> None:
        ...
        errors = _naturalistic_feature_errors(broken, Path("synthetic-broken.json"), load_json(DEFAULT_V1_MANIFEST))
    ...
    def test_profile_realism_rejects_serialized_terse_message(self) -> None:
        ...
        errors = _naturalistic_feature_errors(
            broken, Path("synthetic-serialized-terse.json"), load_json(DEFAULT_V1_MANIFEST)
        )
    ...
    def test_semantic_title_base_duplicate_is_release_error(self) -> None:
        manifest = load_json(DEFAULT_V1_MANIFEST)
        source_path = sorted((ROOT / "data/v1.0/scenarios").rglob("*.json"))[0]
    ...
    def test_rendered_message_grammar_gate_catches_known_b_defects(self) -> None:
        manifest = load_json(DEFAULT_V1_MANIFEST)
        catalog = load_json(ROOT / "data/v1.0/catalog.json")
        scenario_path = sorted((ROOT / "data/v1.0/scenarios").rglob("*.json"))[0]
        broken = copy.deepcopy(load_json(scenario_path))
        for realization in broken["realizations"]:
            if realization["form"] != "naturalistic":
        ...
        errors = _validate_v1_scenario(broken, Path("synthetic-broken.json"), manifest, catalog)
