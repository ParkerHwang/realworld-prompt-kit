#!/usr/bin/env python3
"""Dependency-free validation and quality lint for the v1 broad core."""
...
EXPECTED_SCHEMA = "realworld-prompt-kit.scenario/1.0.0"
EXPECTED_MANIFEST_SCHEMA = "realworld-prompt-kit.manifest/1.0.0"
EXPECTED_CATALOG_SCHEMA = "realworld-prompt-kit.catalog/1.0.0"
...
def _load_contract(manifest_path: Path, root: Path, report: ValidationReport) -> tuple[dict[str, Any], dict[str, Any]] | None:
    ...
    if manifest.get("schema") != EXPECTED_MANIFEST_SCHEMA:
        report.errors.append(
            f"{manifest_path}: unsupported v1 manifest schema {manifest.get('schema')!r}"
        )
    catalog_path = root / manifest.get("catalog_path", "data/v1.0/catalog.json")
    try:
    ...
    if catalog.get("schema") != EXPECTED_CATALOG_SCHEMA:
        report.errors.append(
            f"{catalog_path}: unsupported v1 catalog schema {catalog.get('schema')!r}"
        )
...
def _validate_v1_scenario(
    ...
    if manifest.get("status") == "released" and document.get("status") == "draft":
        errors.append(f"{path}: released v1 scenarios must be reviewed or frozen, not draft")
    ...
    if provenance.get("origin") != "synthetic":
        errors.append(f"{path}: v1 public scenarios must declare origin=synthetic")
...
def validate_v1_repository(
    ...
    for path in paths:
        ...
        scenario_errors = _validate_v1_scenario(
