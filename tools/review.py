#!/usr/bin/env python3
"""Create the integration review record for one sample from every v0.1 block."""
...
def _sample_review(
    ...
    scenario_validation_errors = _validate_scenario(document, path, manifest, catalog)
...
def build_review(manifest_path: Path = DEFAULT_MANIFEST, root: Path = ROOT) -> dict[str, Any]:
    ...
    validation = validate_repository(manifest_path, root)
    try:
    ...
    return {
        "schema": "realworld-prompt-kit.review-report/0.1.0",
        "reviewed_at": REVIEW_DATE,
...
def main() -> int:
    ...
    parser.add_argument("--json-output", type=Path, default=ROOT / "reports/review/review.json")
    parser.add_argument("--markdown-output", type=Path, default=ROOT / "reports/review/review.md")
